...
fn error_event_returns_err() {
    let body = r#"data: {"error": {"message": "Invalid API key", "type": "invalid_request_error"}}"#;
...
#[test]
fn error_event_no_type_field() {
    let body = r#"data: {"error": {"message": "Something went wrong"}}"#;
    let result = parse_sse_response(body);
    assert!(result.is_err());
    let err_msg = format!("{}", result.unwrap_err());
